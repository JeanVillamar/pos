				...
				<?php if (!empty($clients)): ?>
					...
					<?php foreach ($clients as $key => $value): ?>
						...
						<?php if (!empty($order)): ?>
							...
							<option value="<?php echo $value->id_client ?>" 
								<?php if ($order->id_client_order == $value->id_client): ?> selected 
								<?php endif ?>>
								<?php echo urldecode($value->name_client)." ".urldecode($value->surname_client)." ".urldecode($value->dni_client) ?>
							</option>
	...
	<div class="col-5">
		...
		<div class="form-group">
			...
			<div class="input-group">
				...
				<input type="text" readonly class="form-control rounded-start bg-light" 
				id="seller"
				idAdmin="<?php echo $_SESSION["admin"]->id_admin ?>" 
				value="<?php echo urldecode($_SESSION["admin"]->name_admin) ?>">
