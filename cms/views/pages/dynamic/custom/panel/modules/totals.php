...
if(!empty($order)){
	...
	//en caso de que no haya orden creada se inicializan los valores en 0
	$subtotal = 0;
	$discount = 0;
	$tax = 0;
	$granTotal = 0;
...
<h6 class="text-center px-3 py-4 my-3 rounded border-0 w-100 font-weight-bold <?php if (empty($order)): ?> bg-light 
	<?php else: ?> backColor <?php endif ?>" 
		id="granTotal">Gran Total: $ 
		<span granTotal="<?php echo $granTotal ?>"><?php echo number_format($granTotal,2) ?></span>
</h6>